...
__export(src_exports, {
  BeaverClient: () => BeaverClient,
  PostTypes: () => post_types_exports,
  SwipeTypes: () => swipe_types_exports,
  UserTypes: () => user_types_exports
});
...
var Logger = class {
  prefix;
  isLoggingEnabled;
  constructor(prefix = "", isLoggingEnabled = true) {
    this.prefix = prefix;
  ...
  logMessage(method, message, ...args) {
...
var TransactionDataBuilder = class {
  ...
  static restore(data) {
    if (data.version === 2) {
      return new TransactionDataBuilder(parse(TransactionData2, data));
    } else {
      return new TransactionDataBuilder(parse(TransactionData2, transactionDataFromV1(data)));
    }
  }
  /**
...
function parseObject(object2) {
  return {
    ...
    type: object2.type,
    content: object2.bcs?.dataType === "moveObject" ? fromBase64(object2.bcs.bcsBytes) : new Uint8Array(),
...
/** @hidden */
__publicField(Post, "CREATE_ERROR", "unable to create post");
/** @hidden */
__publicField(Post, "UPDATE_ERROR", "unable to update post");
/** @hidden */
...
async function getByID(options) {
  ...
  const { id } = options;
  return apiClient.content.posts[":id"].$get({ param: { id } });
}
...
async function getInteractionsByType(options) {
  const { apiClient } = this.defaults;
...
async function getUserFeed(options) {
  const { apiClient } = this.defaults;
...
async function getUserProfilePosts(options) {
  const { apiClient } = this.defaults;
  const { page, limit, type } = options;
...
async function unrepost(options) {
  const { apiClient, surface } = this.defaults;
...
Object.assign(Post.prototype, moderation_exports);
...
/** @hidden */
__publicField(Swipe, "CREATE_ERROR", "unable to create swipe");
/** @hidden */
__publicField(Swipe, "UPDATE_ERROR", "unable to update swipe");
/** @hidden */
...
__export(retrieval_exports2, {
  getByID: () => getByID2,
  getFeed: () => getFeed2,
  getInteractionsByType: () => getInteractionsByType2,
  getUserFeed: () => getUserFeed2,
  getUserProfileSwipes: () => getUserProfileSwipes
});
async function getByID2(options) {
  const { apiClient } = this.defaults;
  const { id } = options;
  return apiClient.content.swipes[":id"].$get({ param: { id } });
}
async function getFeed2(options) {
  const { apiClient } = this.defaults;
  const { page, limit } = options;
  return apiClient.content.swipes.$get({ query: { page, limit } });
}
async function getInteractionsByType2(options) {
  const { apiClient } = this.defaults;
...
async function getUserFeed2(options) {
  const { apiClient } = this.defaults;
  const { page, limit, type } = options;
...
async function getUserProfileSwipes(options) {
  const { apiClient } = this.defaults;
  const { page, limit } = options;
...
async function delete_2(options) {
  const { apiClient, surface } = this.defaults;
  const { id } = options;
  const payload = JSON.stringify({ id });
  const messageBytes = new TextEncoder().encode(payload);
...
async function update2(options) {
  const { apiClient, surface } = this.defaults;
...
async function unlike2(options) {
  const { apiClient, surface } = this.defaults;
  const { id } = options;
  const payload = JSON.stringify({ id });
  const messageBytes = new TextEncoder().encode(payload);
...
async function repost2(options) {
  const { apiClient, surface } = this.defaults;
...
async function unrepost2(options) {
  const { apiClient, surface } = this.defaults;
  const { id } = options;
  const payload = JSON.stringify({ id });
  const messageBytes = new TextEncoder().encode(payload);
...
async function unsave2(options) {
  const { apiClient, surface } = this.defaults;
  const { id } = options;
  const payload = JSON.stringify({ id });
  const messageBytes = new TextEncoder().encode(payload);
...
Object.assign(Swipe.prototype, moderation_exports2);
...
/** @hidden */
__publicField(User, "UPDATE_ERROR", "unable to update user");
/** @hidden */
__publicField(User, "FETCH_ERROR", "unable to fetch user");
/** @hidden */
__publicField(User, "INTERACTIONS_ERROR", "unable to fetch interactions");
/** @hidden */
__publicField(User, "SUINS_SYNC_ERROR", "unable to sync suins");
/** @hidden */
__publicField(User, "AWARDS_ERROR", "unable to fetch awards");
/** @hidden */
...
async function getInteractions(options) {
  const { apiClient } = this.defaults;
  const { page, limit, type } = options;
  try {
...
async function getAwards2(options) {
  const { apiClient } = this.defaults;
  const { page, limit, type } = options;
  try {
...
async function update3(options) {
  const { apiClient, surface } = this.defaults;
...
var BeaverClient = class {
  ...
  constructor(surface, config) {
    ...
    const contracts = new import_contracts.Contracts({
  ...
  async initialize() {
    ...
    this.defaults.contracts = new import_contracts.Contracts({
  ...
  get post() {
    return new posts_default(this.defaults, this.logger);
  }
  get swipe() {
    return new swipes_default(this.defaults, this.logger);
  }
  get user() {
    return new user_default(this.defaults, this.logger);
  }
};
