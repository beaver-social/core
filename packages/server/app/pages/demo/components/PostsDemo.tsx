...
export default function PostsDemo() {
    ...
    const { data: postArray, fetchNextPage, hasNextPage, refetch: refetchPosts } = beaver.post.getPosts({ perPage: 2 });
    const { mutateAsync: createPost, isPending: isCreating, isSuccess: isCreatedPost } = beaver.post.createPost;
        ...
        <div className="space-y-6">
            ...
            {isAuthenticated ? (
                ...
                <div className="flex gap-2 items-center justify-center border text-grey-600 border-grey-800 rounded-sm p-4">
                    ...
                    <h3 className="text-sm text-center font-medium">Login to create a post</h3>
                </div>
            )}
            ...
            <div className="space-y-4">
                ...
                {hasNextPage && (
                    <Button variant="outline" className="w-full" onClick={() => fetchNextPage()}>Load More</Button>
                )}
            </div>
        </div >
    );
...
function PostCard({ postId }: { postId: number }) {
    const beaver = useBeaver();
    const { data: post, isLoading: postLoading, refetch: refetchPost } = beaver.post.getPostById({ id: postId.toString() });
    const { data: author, isLoading: authorLoading } = beaver.profile.getProfile({
        type: "id",
        value: post?.authorId.toString() || ""
    });
    const { data: interaction, isLoading: interactionLoading, refetch: refetchInteraction } = beaver.post.getUserPostInteraction({ id: postId });
    const { mutate: likePost, isSuccess: likeSuccess } = beaver.post.likePost;
    const { mutate: unlikePost, isSuccess: unlikeSuccess } = beaver.post.unlikePost;
        ...
        >
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center">
                    ...
                    <div>
                        <h3 className="font-bold text-sm md:text-base">@{author?.username || "user"}</h3>
                        <span className="text-muted-foreground text-xs">{moment(post?.createdAt).fromNow()}</span>
                    </div>
                </div>
            </div>
            ...
            >
                <Icon name={interaction?.hasLiked ? "Heart" : "HeartHandshake"} className="size-3.5 md:size-4" />
