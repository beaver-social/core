...
export default function AuthDemo() {
  ...
  const [activeView, setActiveView] = useState<"register" | "login">(
    hasIdentity ? "login" : "register",
  );
    ...
    return (
      <div className="space-y-6">
        <div className="p-4 md:p-6 bg-green-50/30 border border-green-200 rounded-md">
          <h2 className="text-base md:text-lg font-medium text-green-700">
      ...
      {activeView === "login" ? (
        ...
        >
          <div>
            ...
            <p className="text-xs md:text-sm text-muted-foreground mt-1">
              Connect your wallet to login to your Beaver account.
            </p>
          </div>
          <Button
            variant="outline"
          ...
          >
            {isLoginPending ? (
              <>
                <Icon
                  name="Loader"
                  className="mr-2 h-3.5 w-3.5 md:h-4 md:w-4 animate-spin"
                />
                Connecting...
              </>
            ) : isAuthenticated ? (
              <>
                <Icon
                  name="LogOut"
                  className="mr-2 h-3.5 w-3.5 md:h-4 md:w-4"
                />
                Logout
              </>
            ) : (
        ...
        >
          <div>
            ...
            <p className="text-xs md:text-sm text-muted-foreground mt-1">
              Login to your Beaver account with your wallet.
            </p>
          </div>
        </motion.div>
      ) : (
        ...
        >
          <div>
            ...
            <p className="text-xs md:text-sm text-muted-foreground mt-1">
              Create a new Beaver account with your wallet.
            </p>
          </div>
