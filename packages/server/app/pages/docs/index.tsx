...
function DocRedirect() {
  ...
  useEffect(() => {
    ...
    const gettingStartedDoc = metadata.find(
      (doc) => doc.group === "Getting Started",
    );
  ...
  return (
    <div className="flex">
      <Helmet>
        <title>
          {data?.metadata?.title
            ? `${data?.metadata?.title} | Beaver Social Docs`
            : "Documentation | Beaver Social"}
        </title>
    ...
    <HelmetProvider>
      <div className="min-h-screen text-zinc-200 relative">
        <Helmet>
          ...
          <meta
            name="description"
            content="Comprehensive documentation for Beaver Social - the Web3 Social Network Layer built on Sui Blockchain"
          />
        </Helmet>
