...
export default function DocsContent({ data }: DocsContentProps) {
  ...
  const currentDoc = useMemo(
    () => allDocs.find((doc) => doc.id === selectedDoc),
    [selectedDoc, allDocs],
  );
  ...
  const relatedDocs = useMemo(() => {
    ...
    return allDocs
      .filter(
        (doc) => doc.id !== currentDoc.id && doc.group === currentDoc.group,
      )
      .slice(0, 2);
  }, [currentDoc, allDocs]);
  ...
  const { prevDoc, nextDoc } = useMemo(() => {
    ...
    const groups = Array.from(
      new Set(allDocs.map((doc) => doc.group).filter(Boolean)),
    );

    // Create a flat array of all documents organized by groups
    const allDocsFlattened = groups.flatMap(
      (group) => docsByGroup.get(group) || [],
    );

    const currentIndex = allDocsFlattened.findIndex(
      (doc) => doc.id === selectedDoc,
    );
    const prev = currentIndex > 0 ? allDocsFlattened[currentIndex - 1] : null;
    const next =
      currentIndex < allDocsFlattened.length - 1
        ? allDocsFlattened[currentIndex + 1]
        : null;
      ...
      >
        {prevDoc && (
          ...
          >
            <Icon
              name="ArrowLeft"
              className="h-5 w-5 mt-0.5 text-zinc-500 group-hover:text-blue-400 transition-colors"
            />
            <div>
