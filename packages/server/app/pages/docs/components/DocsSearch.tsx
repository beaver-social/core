...
export default function DocsSearch({ className = "", data }: DocsSearchProps) {
                ...
                >
                  ...
                  {doc.group && (
                    <div className="flex items-center gap-2 text-xs">
                      <Icon
                        name={
                          (groupIcons.get(doc.group) as keyof typeof icons) ||
                          "Book"
                        }
                        className="h-3 w-3 text-blue-400"
                      />
                      <span className="text-zinc-400">{doc.group}</span>
                    </div>
                  )}
                  {doc.description && (
