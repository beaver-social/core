...
export default function DocsTree({ className = "", data }: DocsTreeProps) {
  ...
  useEffect(() => {
    ...
    if (
      currentDoc &&
      currentDoc.group &&
      !expandedSections.includes(currentDoc.group)
    ) {
          ...
          <div key={groupName} className="mb-4">
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`p-1.5 rounded-md ${isActive ? "bg-blue-500/20" : "bg-zinc-800/50"}`}
                >
                  <Icon
                    name={groupIcon}
                ...
                <span
                  className={`font-medium ${isActive ? "text-blue-300" : "text-zinc-300"}`}
                >
                  {groupName}
                </span>
              </div>
              <motion.div
                animate={{
                  rotate: expandedSections.includes(groupName) ? 180 : 0,
                }}
