...
export function CodeShowcase() {
    ...
    const [copied, setCopied] = React.useState({ react: false, node: false, flutter: false });
                ...
                <div className="relative z-10 rounded-xl bg-zinc-900/80 backdrop-blur-sm shadow-xl overflow-hidden border border-zinc-800/50">
                    <div className="flex items-center justify-between p-4 border-b border-zinc-800/50">
                        ...
                        <div className="px-3 py-1 rounded-md bg-zinc-800 text-zinc-400 text-xs">
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">Beaver</span>Social SDK
                        </div>
                    </div>

                    <Tabs defaultValue="react" className="w-full" value={activeTab} onValueChange={(value) => setActiveTab(value as "react" | "node" | "flutter")}>
                        ...
                        <TabsContent value="react" className="relative">
                            <div className="absolute top-2 right-2 z-10">
                                ...
                                >
                                    {copied.react ? <Check className="h-4 w-4 text-green-400" /> : <Copy className="h-4 w-4" />}
                                </button>
                            </div>
                        ...
                        <TabsContent value="node" className="relative">
                            <div className="absolute top-2 right-2 z-10">
                                <button
                                    ...
                                    className="p-2 rounded-md bg-zinc-800 text-zinc-400 hover:text-zinc-100 transition-colors"
                                >
                                    {copied.node ? <Check className="h-4 w-4 text-green-400" /> : <Copy className="h-4 w-4" />}
                                </button>
                            </div>
                            <pre className="p-4 text-sm md:text-md overflow-x-auto font-mono text-zinc-300">
                                {nodeCode}
                        ...
                        <TabsContent value="flutter" className="relative">
                            <div className="absolute top-2 right-2 z-10">
                                <button
                                    ...
                                    className="p-2 rounded-md bg-zinc-800 text-zinc-400 hover:text-zinc-100 transition-colors"
                                >
                                    {copied.flutter ? <Check className="h-4 w-4 text-green-400" /> : <Copy className="h-4 w-4" />}
                                </button>
                            </div>
                            <pre className="p-4 text-sm md:text-md overflow-x-auto font-mono text-zinc-300">
                                {flutterCode}
