...
type AspectRatio = 'square' | 'portrait' | 'landscape' | 'banner' | 'wide' | 'custom';
...
const aspectRatioOptions: Record<AspectRatio, { value: number, label: string, icon: string }> = {
...
export default function ImageCropDialog({
    ...
    allowedAspectRatios = ['square', 'portrait', 'landscape', 'banner', 'wide', 'custom']
}: Props) {
    ...
    const currentAspectRatioValue = aspectRatio === 'custom'
                    ...
                    <div className="flex flex-col gap-2 py-4">
                        ...
                        >
                            {filteredAspectRatios.map((ratio) => (
                                <ToggleGroupItem key={ratio} value={ratio} aria-label={aspectRatioOptions[ratio].label}>
                                    <Icon name={aspectRatioOptions[ratio].icon as any} className="mr-2 size-4" />
                                    <span className="hidden sm:inline">{aspectRatioOptions[ratio].label}</span>
                                </ToggleGroupItem>
                            ))}
                        </ToggleGroup>
                    </div>
