...
// Define the timezones table
export const timezones = table(
  "timezones",
  {
    id: t.int().primaryKey(),
    label: t.text().notNull(),
    value: t.text().notNull().unique(),
    offset: t.int().notNull(), // offset in minutes
    offsetStr: t.text().notNull(), // human-readable offset string
    ...timestamps,
  },
  (table) => [
...
export const timezonesEnum = timezoneData.map((t) => t.id);

// Helper functions remain the same but reference the timezoneData array
const getTimezoneById = (id: number) => {
  return timezoneData.find((tz) => tz.id === id) || null;
};
